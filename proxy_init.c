...
void key_init(void)
{
    ...
    if ((keyfile_count == 0) && (keyinfo_count == 0)) {
        privkey[0].ec = gen_EC_key(DEF_CURVE, 0, NULL);
        if (!privkey[0].key) {
            exit(ERR_CRYPTO);
        }
        privkey_type[0] = KEYBLOB_EC;
        key_count = 1;
    ...
    if ((proxy_type == RESPONSE_PROXY) && (v4_ecdh_curve != 0)) {
        v4_dhkey.ec = gen_EC_key(v4_ecdh_curve, 1, NULL);
        if (!v4_dhkey.key) {
