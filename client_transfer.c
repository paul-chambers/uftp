                        ...
                        struct tfmcc_ack_info_he *tfmcc)
{
    ...
    glog4(group, "ccrate=" F_i64, ccrate);
    ...
    if (group->version == UFTP4_VER_NUM) {
        tfmcc->tstamp_hi = htonl((uint32_t)send_time.tv_sec);
        tfmcc->tstamp_lo = htonl((uint32_t)send_time.tv_usec);
    } else {
        send_time_us = tv_to_usec(send_time);
        tfmcc->tstamp_hi = htonl((send_time_us & 0xFFFFFFFF00000000ULL) >> 32);
        tfmcc->tstamp_lo = htonl(send_time_us & 0x00000000FFFFFFFFULL);
    }
}
...
void print_result_status(struct group_list_t *group)
{
    ...
    fprintf(status_file,
            "RESULT;%04d/%02d/%02d-%02d:%02d:%02d;%08X;%08X;%s;" F_u64 "KB",
            ...
            group->fileinfo.name, group->fileinfo.size / 1024);
...
void init_tfmcc_fb_round(struct group_list_t *group, uint16_t new_ccseq)
{
    ...
    glog3(group,"Starting feedback round %u: backoff = %.3f, initrate = " F_i64,
                      ...
                      unsigned meslen, struct timeval rxtime)
{
    ...
    if (group->version == UFTP4_VER_NUM) {
        group->last_server_ts.tv_sec = ntohl(cong_ctrl->tstamp_hi);
        group->last_server_ts.tv_usec = ntohl(cong_ctrl->tstamp_lo);
    } else {
        time_us = (int64_t)ntohl(cong_ctrl->tstamp_hi) << 32;
        time_us |= ntohl(cong_ctrl->tstamp_lo);
        group->last_server_ts = usec_to_tv(time_us);
    }
